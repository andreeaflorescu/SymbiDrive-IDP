'''
Created on Apr 23, 2015

@author: andreea
'''
class SocialProfile(object):
    FACEBOOK = "Facebook"
    GOOGLE = "Google+"

class ExitCode(object):
    USER_ALREADY_REGISTERED = "USER_ALREADY_REGISTERED"
    USER_REGISTER_WITH_OTHER_PROFILE = "USER_REGISTER_WITH_OTHER_PROFILE"
    USER_REGISTER_ON_OTHER_DEVICE = "USER_REGISTER_ON_OTHER_DEVICE"
    USER_REGISTER_MODIFIED_TOKEN = "USER_REGISTER_MODIFIED_TOKEN"
    USER_CREATED = "USER_CREATED"
    INVALID_USER = "INVALID_USER"
    DELETED_USER = "DELETED_USER"
    
    PROFILE_UPDATED = "PROFILE_UPDATED"
    
    RATING_ADDED = "RATING_ADDED"
    INVALID_RATING = "INVALID_RATING"
    
    FEEDBACK_ADDED = "FEEDBACK_ADDED"
    INVALID_FEEDBACK = "INVALID_FEEDBACK"
    
    POOL_ADDED = "POOL_ADDED"
    INVALID_POOL_PARAMETER = "INVALID_POOL_PARAMETER"
    INVALID_POOL = "INVALID_POOL"
    POOL_DELETED = "POOL_DELETED"
    PASSENGER_ADDED_TO_POOL = "PASSENGER_ADDED_TO_POOL"
    PASSENGER_DELETED_FROM_POOL = "PASSENGER_DELETED_FROM_POOL"
    
    ROUTE_ADDED = "ROOT_ADDED"

class IndexName(object):
    LOCATION_INDEX = "LOCATION_INDEX"
    ROUTE_INDEX = "ROUTE_INDEX"
